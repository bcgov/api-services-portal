...
const flattenNestedErrors = (error) =>
  (error.errors || []).reduce(
    (errors, nestedError) => [
      ...errors,
      ...[
        duplicateError(nestedError, ['errors']),
        ...flattenNestedErrors(nestedError),
      ].map((flattenedError) => {
...
const formatError = (error) => {
  ...
  try {
    ...
    if (isApolloErrorInstance(originalError)) {
      ...
      const exception =
        originalError || (error.extensions && error.extensions.exception);
      if (exception) {
        const pinoError = {
        ...
        if (pinoError.errors) {
          pinoError.errors = flattenNestedErrors(exception).map(
            safeFormatError
          );
        }
