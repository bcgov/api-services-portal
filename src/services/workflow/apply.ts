...
import { isUpdatingToIssued, isUpdatingToRejected, isRequested } from './common'
        ...
        try {
            ...
            if (requestDetails.productEnvironment.approval == false) {
                ...
                const setup = <SetupAuthorizationInput> {
                    ...
                    controls: 'controls' in requestDetails ? JSON.parse(requestDetails.controls) : {},
                    environmentName: requestDetails.productEnvironment.name,
                    environmentAppId: requestDetails.productEnvironment.appId,
                    credentialIssuerId: requestDetails.productEnvironment.credentialIssuer.id,
                ...
                await updateAccessRequestState (context, requestDetails.id, { isApproved: true, isIssued: true, isComplete: true })

                message.text = "received credentials (immediate approval)"    
            }

            await recordActivity (context, operation, 'AccessRequest', updatedItem.id, message.text, "success", JSON.stringify(originalInput))
...
async function setupAuthorizationAndEnable (context: any, setup: SetupAuthorizationInput) {
    ...
    if (flow == 'client-credentials') {
        ...
        const issuer = await lookupCredentialIssuerById(context, setup.credentialIssuerId)
        const issuerEnvConfig: IssuerEnvironmentConfig = getIssuerEnvironmentConfig(issuer, setup.environmentName)
        ...
        const controls: RequestControls = {...{defaultClientScopes:[]}, ...setup.controls}
        ...
        await kongApi.updateConsumerACLByNamespace (kongConsumerPK, ns, [ setup.environmentAppId ], true)
