...
export const DeleteAccess = async (context: any, keys: any) => {
  ...
  if ('application' in keys) {
    ...
    svc.consumer != null &&
      ...
      (await deleteRecord(context, 'GatewayConsumer', { id: svc.consumer.id }, [
        'id',
        'extForeignKey',
        'customId',
      ]));
    ...
    if (
      ...
      // Synchronously do the deletion of the backend IdP and Kong
      await kongApi
        .deleteConsumer(svc.consumer.extForeignKey)
        .then(async () => {
        ...
        .catch((err) => {
          logger.error('[DeleteAccessFromIdP] Error deleting');
          logger.error('[DeleteAccessFromIdP] Reason %s', err);
        });
    }
  }
};
