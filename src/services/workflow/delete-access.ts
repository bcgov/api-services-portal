...
export const DeleteAccess = async (context: any, operation: any, keys: any) => {
    ...
    if ('application' in keys) {
        ...
        */
        ...
        svc.consumer != null && svc.consumerType == 'client' && deleteRecord(context, 'GatewayConsumer', { id: svc.consumer.id }, ['id', 'extForeignKey', 'customId'])

        svc.consumer != null && svc.consumerType == 'client' && kongApi.deleteConsumer(svc.consumer.extForeignKey)
                .then((async () => {
