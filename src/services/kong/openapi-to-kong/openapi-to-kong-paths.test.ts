...
describe("trailingSlash: optional", () => {
  ...
  expect(staticR.isRegex).toBe(true, "static path becomes regex when optional slash needed");
...
describe("convertPaths (bulk)", () => {
  const results = convertPaths(["/pets", "/pets/{petId}", "/users/{uid}/orders/{oid}"]);
