...
export class KeycloakPermissionTicketService {
    ...
    public async getPermissionTicket (resourceId : string, requesterId : string, scopeId : string) : Promise<PermissionTicket> {
    ...
    public async updatePermissionGrantedFlag (body : PermissionTicket ) : Promise<void> {
    ...
    public async approvePermission (resourceId : string, requesterId : string, scopeId : string) : Promise<void> {
        const perm : PermissionTicket = await this.getPermissionTicket (resourceId, requesterId, scopeId)
        perm.granted = true
        await this.updatePermissionGrantedFlag (perm)
    }    

    public async createPermission (resourceId : string, requesterId : string, granted : boolean, scopeName : string) : Promise<PermissionTicket> {
        const url = `${this.issuerUrl}/authz/protection/permission/ticket`
    ...
    public async createOrUpdatePermission (resourceId : string, requesterId : string, granted : boolean, scopeName : string) : Promise<PermissionTicket> {
        ...
        if (perms.length == 0) {
            return await this.createPermission(resourceId, requesterId, granted, scopeName)
            ...
            await this.updatePermissionGrantedFlag({
                ...
                granted: granted,
                scopeName: perm.scopeName} as PermissionTicket)
            return perm
        }
    }    
