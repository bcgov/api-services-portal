...
export class MigrationFromV1 {
    ...
    public async migrate (definition: V1Definition[]) {
        ...
        const noauthContext =  this.keystone.createContext({ skipAccessControl: true })
        ...
        const resOwnerResources = await kcprotectApi.listResources({ owner: tokenResult.clientUuid, type: prodEnv.credentialIssuer.resourceType} as ResourceSetQuery)

        const kcpermApi = new KeycloakPermissionTicketService (tokenResult.issuer, tokenResult.accessToken)

        const resourceScopes = JSON.parse(prodEnv.credentialIssuer.availableScopes)

        return Promise.all(definition.map(async (def: V1Definition) => {
            ...
            if (found.length == 0) {
                ...
                const existingAccounts = await keystoneApi.lookupServiceAccessesByNamespace(noauthContext, def.namespace)
                const isExistingAccount = (nm:string) => existingAccounts.filter((sa:any) => sa.consumer.username === nm).length != 0

                for (const svc of def.service_accounts) {
                    if (isExistingAccount(svc.clientId)) {
                        logger.info(" > Skipped [%s] Service Account", svc.clientId)
                    } else {
                        const result = await CreateServiceAccount(noauthContext, productEnvironmentSlug, def.namespace, svc.clientId)
                        logger.info(" > Created [%s] Service Account %j", svc.clientId, result)
                    }
                }
                ...
                for ( const user of def.admin_membership) {
                    ...
                    for (const scope of ['Namespace.Manage']) {
                        const permission = await kcpermApi.createOrUpdatePermission (newresource.id, user.id, granted, scope)
