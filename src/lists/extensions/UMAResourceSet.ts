...
import { isUserBasedResourceOwners, getSuitableOwnerToken, getResourceSets, getEnvironmentContext } from './Common'
  ...
  extensions: [
      (keystone : any) => {
        keystone.extendGraphQLSchema({
            ...
            queries: [
              {
                schema: 'allResourceSets(prodEnvId: ID!, type: String): [UMAResourceSet]',
                resolver: async (item : any, args : any, context : any, info : any, { query, access } : any) => {
                    ...
                    const resourcesApi = new UMAResourceRegistrationService (envCtx.openid.issuer, envCtx.accessToken)
                    return await resourcesApi.listResourcesByIdList(resourceIds)
                },
                access: EnforcementPoint,
              },
              {
                schema: 'getResourceSet(prodEnvId: ID!, resourceId: String!): UMAResourceSet',
                resolver: async (item : any, args : any, context : any, info : any, { query, access } : any) => {
                    const envCtx = await getEnvironmentContext(context, args.prodEnvId, access)
