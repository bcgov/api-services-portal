    ...
    (keystone) => {
      keystone.extendGraphQLSchema({
        ...
        mutations: [
          ...
          {
            schema:
              'updateGatewayConsumerPlugin(id: ID!, pluginExtForeignKey: String!, plugin: String!): GatewayConsumer',
            resolver: async (item, args, context, info, { query, access }) => {
              const noauthContext = keystone.createContext({
                skipAccessControl: true,
              });
          ...
          {
            schema:
              'deleteGatewayConsumerPlugin(id: ID!, pluginExtForeignKey: String!): GatewayConsumer',
            resolver: async (item, args, context, info, { query, access }) => {
              const noauthContext = keystone.createContext({
                skipAccessControl: true,
              });
