...
const Metric = ({service}) => {
    ...
    // last 5 days
    const days = [0,1,2].map(d => toDate((new Date().getTime()-(24*60*60*1000*d))))
        ...
        <Stack direction="row" spacing={0} align="flex-start" justify="flex-start" style={{backgroundColor:'white',padding:'5px'}}>
            {metrics.map(mm => 
                JSON.parse(mm.values).map(v => (
                    <Tooltip label={`${dt.format(new Date(Number(v[0])*1000))} : ${Math.round(Number(v[1]))}`} aria-label="">
                        <p><span style={{display:'inline-block',width:'2px',height:'30px',backgroundColor:color(v[1]),marginLeft:'1px'}}></span></p>
                    </Tooltip>
                ))
        )}
        </Stack>
