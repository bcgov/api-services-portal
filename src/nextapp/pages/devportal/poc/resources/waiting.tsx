...
function List({ data, state, granted, loginUserSub, revokeAccess }) {
    ...
    switch (state) {
      ...
      case 'loaded': {
        ...
        const list = data
            .filter(item => item.requester == loginUserSub)
                ...
                <Table variant="simple">
                    ...
                    {list.map((item, index) => (
                        <Tr key={item.id}>
                            ...
                            <Td>
                                {granted ? false : (
                                    <Button colorScheme="red" size="sm" onClick={() => revokeAccess(item.scopes.map(s => s.ticketId))}>Cancel</Button>
                                )} 
                            </Td>
                        </Tr>
                    ))}
