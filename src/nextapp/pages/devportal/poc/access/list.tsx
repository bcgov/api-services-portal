...
const TEMPLATES = {
  'kong-acl-only': [TEMPL_API_KEY_CURL, TEMPL_API_KEY_RESTISH],
...
function List({ data, state, refetch, cancelRequest }) {
  switch (state) {
    ...
    case 'loaded': {
      ...
      const products = [
        ...new Set(
          data.myServiceAccesses
            .map((item, index) => item.productEnvironment?.product.name)
            .filter((p) => p != null)
        ),
      ];
              ...
              <Table variant="simple">
                ...
                <Tbody>
                  {data.myServiceAccesses
                    .filter(
                      (access) =>
                        access.productEnvironment?.product.name == product
                    )
                    .map((item, index) => {
