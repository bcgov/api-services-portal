...
async function replay({workingPath, destinationUrl}) {
    const exceptions = []
    const xfer = transfers(workingPath, null, exceptions)

    // Now, send to portal
    await xfer.concurrentWork(loadProducer(xfer, destinationUrl))
}

async function loadProducer (xfer, destinationUrl) {
