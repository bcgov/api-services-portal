...
// describe('Apply Rate Limiting for JWT Generated Key Pair authenticator Authorization Profile', () => {
//   const login = new LoginPage()
//   const home = new HomePage()
//   const consumers = new ConsumersPage()
...
//   it('set api rate limit to the consumer which is set for JWT Generated Key Pair authenticator Authorization Profile', () => {
//     cy.readFile('cypress/fixtures/state/store.json').then((store_res) => {
...
//           home.useNamespace(clientCredentials.namespace);
//           cy.visit(consumers.path);
//           consumers.clickOnTheTheConsumerID(jwkCred.clientId)
//           consumers.setRateLimiting('3')
//         })
//       })
//     })
...
//         cy.getTokenUsingJWKCredentials(store_res.jwtkeypaircredentials, privateKey).then((response) => {
...
// describe('Regenerate Credential for Client Credentials- JWT Generated Key Pair authenticator Authorization Profile', () => {
//   const login = new LoginPage()
//   const myAccessPage = new MyAccessPage()
...
//   it('Regenrate credential JWT Generated Key Pair authenticator Authorization Profile', () => {
//     cy.visit(myAccessPage.path)
//     cy.get('@developer').then(({ clientCredentials }: any) => {
//       let jwtkp = clientCredentials.jwtKeyPair
//       myAccessPage.regenerateCredential(jwtkp.product.environment, jwtkp.application.name)
//       myAccessPage.clickOnGenerateSecretButton()
//       cy.contains('Client ID').should('be.visible')
//       cy.contains('Signing Private Key').should('be.visible')
//       cy.contains('Signing Public Certificate').should('be.visible')
//       cy.contains('Token Endpoint').should('be.visible')
//       myAccessPage.saveJwtKeyPairCredentials(true)
//     })
//   })

//   it('Make sure that the old JWT Generated Key Pair credentials is disabled ', () => {
//     cy.readFile('cypress/fixtures/state/store.json').then((store_res) => {
//       cy.readFile('cypress/fixtures/state/jwtGenPrivateKey.pem').then((privateKey) => {
//         cy.getTokenUsingJWKCredentials(store_res.jwtkeypaircredentials, privateKey).then((response) => {
//           let token = response.body.access_token
...
//   it('Verify that rate limiting is applied correctly to the consument', () => {
//     cy.readFile('cypress/fixtures/state/regen.json').then((store_res) => {
//       cy.readFile('cypress/fixtures/state/jwtReGenPrivateKey.pem').then((privateKey) => {
//         cy.getTokenUsingJWKCredentials(store_res.jwtkeypaircredentials, privateKey).then((response) => {
//           let token = response.body.access_token
//           cy.get('@apiowner').then(({ clientCredentials }: any) => {
//             cy.makeKongRequest(clientCredentials.serviceName, 'GET', token).then((response) => {
